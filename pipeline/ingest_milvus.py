...
def insert_chunks(
    ...
    """
    # VARCHAR field limits (must match schema in create_collection)
    _VARCHAR_LIMITS = {
        "id": 128,
    ...
    def _trunc(value: str, field: str) -> str:
        """Truncate a string to the schema byte limit for the given field.

        Milvus VARCHAR max_length counts UTF-8 bytes, not characters.
        We truncate by bytes, taking care not to split a multi-byte char.
        """
        limit = _VARCHAR_LIMITS.get(field, 512)
        encoded = value.encode("utf-8")
        if len(encoded) <= limit:
            return value
        # Truncate at byte boundary, leaving room for "..."
    ...
    for start in range(0, total, batch_size):
        ...
        for chunk, embedding in zip(batch_chunks, batch_embeds):
            ...
            if (
                len(section) > _VARCHAR_LIMITS["section"]
                or len(subsection) > _VARCHAR_LIMITS["subsection"]
            ):
                truncated_count += 1
            ...
            row = {
                ...
                "chapter_title": _trunc(chunk["chapter_title"], "chapter_title"),
                "section": _trunc(section, "section"),
                "subsection": _trunc(subsection, "subsection"),
    ...
    if truncated_count:
        print(f"[ingest]   Truncated section/subsection in {truncated_count} chunks")
    return inserted
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Path to cache embeddings as a pickle file. "
        "If the file exists and chunk count matches, embeddings are loaded "
        "from cache instead of recomputed. Otherwise, embeddings are computed "
    ...
    # Embed (with optional cache to avoid recomputing on retries)
    embeddings = None
    cache_path = args.embed_cache

    if cache_path and cache_path.exists():
        print(f"[ingest] Loading cached embeddings from: {cache_path}")
        with open(cache_path, "rb") as f:
            embeddings = pickle.load(f)
        if len(embeddings) != len(chunks):
            print(
                f"[ingest] WARNING: Cache has {len(embeddings)} embeddings but "
                f"{len(chunks)} chunks — recomputing",
                file=sys.stderr,
            )
            embeddings = None
        else:
            print(f"[ingest]   Loaded {len(embeddings)} cached embeddings")
    ...
    if embeddings is None:
        ...
        if cache_path:
            print(f"[ingest] Saving embeddings to cache: {cache_path}")
            with open(cache_path, "wb") as f:
                pickle.dump(embeddings, f)
            size_mb = cache_path.stat().st_size / (1024 * 1024)
            print(f"[ingest]   Cache saved ({size_mb:.1f} MB)")
